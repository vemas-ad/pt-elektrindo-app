...
// ==============================================================
// 🔧 CONFIGURATION - PASTIKAN ENV VARIABLES SESUAI
// ==============================================================

// Primary account (AKUN 1 - MAIN)
const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL!;
const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!;

// Secondary account (AKUN 2 - BACKUP) 
...
// Hanya log di client side untuk menghindari hydration error
if (typeof window !== 'undefined') {
  console.log("🔧 ========== SUPABASE DUAL STORAGE INITIALIZATION ==========");
}

// Validasi primary account
if (!supabaseUrl || !supabaseAnonKey) {
  // Jangan throw error di client side untuk menghindari hydration error
  if (typeof window !== 'undefined') {
    console.error("❌ PRIMARY Supabase environment variables are missing!");
  }
}
...
// Create secondary client dengan error handling
export let supabase2: ReturnType<typeof createClient> | null = null;
let secondaryEnabled = false;

// PERBAIKAN: Cek apakah environment variables untuk secondary benar-benar valid
const hasValidSecondaryConfig = supabaseUrl2 && supabaseAnonKey2 && 
...
if (hasValidSecondaryConfig) {
  try {
    ...
    if (typeof window !== 'undefined') {
      console.log("✅ SECONDARY Supabase client initialized");
      // Test connection ke secondary - hanya jika di client side
      setTimeout(() => {
        testSecondaryConnection().catch(() => {
          // Silent catch untuk menghindari error di console
        });
      }, 1000);
    }
    ...
    if (typeof window !== 'undefined') {
      console.error("❌ Failed to initialize secondary client:", error.message);
    }
    supabase2 = null;
  ...
  if (typeof window !== 'undefined') {
    console.warn("⚠️ SECONDARY Supabase environment variables are missing or invalid");
  }
  supabase2 = null;
  secondaryEnabled = false;
}
